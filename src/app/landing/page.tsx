    ...
    <section className="flex justify-around pt-44  ">
      <div className="w-1/2 ">
        ...
        <p className="text-gray-500 w-5/6 mt-4">
          Comunicate, collaborate, and celebrate moments wherever you are with
          meet clone
        </p>
        <div className="mt-10 flex justify-between gap-8">
          <Button asChild className="bg-cyan-700 p-6 w-48">
            <Link href={"/room/" + generateRoomId()}>
              <VideoIcon className="mr-4" /> New Meet
            </Link>
          </Button>
          <div
            className={`flex items-center border-2 border-gray-500 rounded-md ${focused ? "border-blue-700" : "border-black"} h-[3rem]`}
          >
            <label htmlFor="ID">
              <KeyboardIcon
                width={50}
                height={50}
                className={`mr-4 p-2 transition-all ${focused ? "text-blue-500" : "text-black"}`}
              />
            </label>
            <input
